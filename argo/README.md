...
**Secrets:** no plaintext committed. `connect40-sealedsecret.yaml` holds only ciphertext
(sealed strict-scope for namespace sdk-apps-dev); the controller unseals it into
`Secret connect40-secrets`, consumed via `envFrom`. Non-secret env (incl. public
`INTERACTIVE_KEY`) lives in `connect40-config`.

## Environment

| Env | Service | Namespace | Host | Health |
| --- | ------- | --------- | ---- | ------ |
| `dev` | `connect40` | `sdk-apps-dev` | connect40-dev-topia.topia-rtsdk.com | `/api/system/health` |
